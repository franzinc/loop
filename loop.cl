...
;;;>      +1-617-221-1000
;;;
;;; Copyright (c) 1989-2014 Franz Inc, Oakland, CA
;;; All rights reserved.
;;; 
;;; Permission to use, copy, modify and distribute this software and its
;;; documentation for any purpose and without fee is hereby granted,
;;; provided that the Franz copyright notice appear in all copies and that
;;; both that copyright notice and this permission notice appear in
;;; supporting documentation.  The name "Franz" may not be used in
;;; advertising or publicity pertaining to distribution of the software
;;; without specific, written prior permission.  Notice must be given in
;;; supporting documentation that copying distribution is by permission of
;;; Franz  Franz makes no representations about the suitability of this
...
(defstruct (loop-universe
  ...
  implicit-for-required				;see looph-ack-iteration
  )
...
;; [bug21384]: New destructuring, without the fussy-car calls
;; (this may eventually be generalizable as a more-lax destructuring
;; macro)

(defmacro destructuring-bind-1 (arglist expr &rest body)
  ;; user function that binds and destructures a given form. This one is
  ;; different than destructuring-bind in that it does not use fussy-car.
  (lambdascan-process 0 arglist expr expr body nil nil t t nil))



;; [bug21384]: Reinstate this, with a simplification
(defun loop-build-destructuring-bindings (crocks forms)
  (if crocks
      (let ((*ignores* ()))
	(declare (special *ignores*))
	`((destructuring-bind-1 ,(subst-gensyms-for-nil (car crocks))
	      ,(cadr crocks)
	    (declare (ignore ,@*ignores*))
	    ,@(loop-build-destructuring-bindings (cddr crocks) forms))))
      forms))

;; [bug21384]: This one tries to load the compiler.
#+ignore-bad-def
(defun loop-build-destructuring-bindings (crocks forms env)
  ;; We need this more elaborate definition in order to satisfy
  ;; the ANS [bug19927].  The destructuring specified for loop 
  ;; variable bindings is more lenient than the destructuring 
  ;; done by destructuring-bind.
  (if crocks
      (let ((*ignores* ()) (var (gensym)))
	(declare (special *ignores*))
	(if (comp::call-compiler-optimization-switch
	     comp::verify-non-generic-switch env)
	    `((compiler-let
	       ((,var *enable-package-locked-errors*))
	       (compiler-let
		((*enable-package-locked-errors* nil))
		(macrolet ((car-fussy (x y) (declare (ignore y)) (list 'car x)))
		  ;; destructuring-bind emits calls to car-fussy 
		  ;; We override the definition with the more lenient
		  ;; and correct use of car.  [bug19927]
		  ;; The compiler-let is needed since car-fussy is in the
		  ;; locked excl package.
		  (destructuring-bind ,(subst-gensyms-for-nil (car crocks))
		      ,(cadr crocks)
		    (declare (ignore ,@*ignores*))
		    (macrolet ((car-fussy (x y) (list 'car-fussy-restored x y)))
		      ;; The body forms must see the original definition of
		      ;; car-fussy in case there are any uses of destructuring-bind
		      ;; in the user code.  [bug19927]
		      (compiler-let
		       ((*enable-package-locked-errors* ,var))
		       ,@(loop-build-destructuring-bindings (cddr crocks) forms))))))))
	  ;; When verify-non-generic-switch is nil, car-fussy is equivalent 
	  ;; to car and the redefinitions are not necessary.
	  `((destructuring-bind ,(subst-gensyms-for-nil (car crocks))
		,(cadr crocks)
	      (declare (ignore ,@*ignores*))
	      ,@(loop-build-destructuring-bindings (cddr crocks) forms)))))
...
(defun loop-translate (*loop-source-code* *loop-macro-environment* *loop-universe*)
  (let ((*loop-original-source-code* *loop-source-code*)
	...
	(let ((vars (first entry))
      ...
      (if *loop-names*
	  (do () ((null *loop-names*) answer) ; [bug21898]
	    (setq answer `(block ,(pop *loop-names*) ,answer)))
	  `(block nil ,answer)))))


(defvar *loop-src-step-prep* nil)
(defvar *loop-src-end-prep* nil)
(defvar *loop-src-end-form* nil)

(defun loop-iteration-driver ()
  (do () ((null *loop-source-code*))
    (let ((keyword (car *loop-source-code*))
	  (tem nil)
	  (*loop-src-step-prep* nil)
	  (*loop-src-end-prep* nil)
	  (*loop-src-end-form* nil))
  ...
  (let ((name (loop-pop-source)))
    ...
    (when *loop-names*
      (loop-error "You may only use one NAMED clause in your loop: NAMED ~S ... NAMED ~S."
		  (car *loop-names*) name))
    (setq *loop-names* (cons name nil)))) ; [bug21898]
...
(defun loop-do-for ()
  (let* ((var (or (loop-pop-source) (loop-gentemp 'loop-do-for-anon-)))
    ...
    (apply (car tem) var first-arg data-type keyword (cdr tem))))
	  ...
	  (with-normal-consing
		...
		(if (symbolp form)
			...
			(setq type 'real))
		      type)
		  'real)))))
    ;;[bug21107]: Fix the lower end of the type declaration for the iteration variable
    (let* ((ntype (normalize-type type :environment *loop-macro-environment*))
	   (low (and (consp ntype) (numberp (second ntype)) (second ntype)))
	   (hi (third ntype)))
      (when (and low (>= low 0))
	(setq type `(,(car type) -1 ,hi))))
...
;;; phase.
(defun loop-ansi-for-equals (var val data-type orig-kwd)
  ...
  (let ((sequence-var (loop-gentemp 'loop-in-sequence-sequence-))
	...
	(loop-constant-fold-if-possible val)
      ...
      (let* ((length 0)
	     (length-form
	      (cond ((not constantp)
		     (let ((v (loop-gentemp 'loop-in-sequence-limit-)))
		       (push `(unless (consp ,sequence-var)
				(setq ,v (length ,sequence-var))
				(setq ,index-var 0))
			     *loop-prologue*)
		       (loop-make-variable v 0 'adim)))
		    (t (setq length (length sequence-value)))))
	     (first-test (cond ((not constantp)
				`(if (eq ,index-var -1)
				     (null ,sequence-var)
				   (>= ,index-var ,length-form)))
...
(defun loop-for-on (var val data-type orig-kwd)
  (declare (ignore orig-kwd))
...
;;; the prologue, etc.
(defun loop-for-being (var val data-type orig-kwd)
  (declare (ignore orig-kwd))
  ...
  (flet ((in-group-p (x group) (car (loop-tmember x group))))
	...
	(when (setq this-prep (in-group-p token group))
      ...
      (cond (this-group
	     ...
	     (loop-pop-source)
	     (push (list this-prep (loop-get-form) token) prepositional-phrases))
...
(defun loop-sequencer (indexv indexv-type indexv-user-specified-p
   ...
   (let ((endform nil)				;Form (constant or variable) with limit value.
     ...
     (do ((l prep-phrases (cdr l)) (prep) (form) (orig-prep) (odir)) ((null l))
       (setq prep (caar l) form (cadar l) orig-prep (caddar l))
       (case prep
	 ((:of :in)
	  (setq sequencep t)
	 ...
	 ((:upto :to :downto :above :below)
	  ...
	  (setq *loop-src-end-prep* orig-prep)
	  (setq *loop-src-end-form* (cadar l)))
	 (:by
...
(defun make-ansi-loop-universe (extended-p mode)
  (let ((w (make-standard-loop-universe
	     ...
	     :for-keywords '((= (loop-ansi-for-equals))
			     ...
			     (in (loop-for-in))
			     (on (loop-for-on))
			     (in-sequence (loop-for-in-sequence))
